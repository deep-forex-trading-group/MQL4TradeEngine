...
public:
   ...
   bool AddOneOrderByStepPip(int direction, double StepPip, double Lot) {
      ...
      if (direction == 1) {
               ...
               Print("create sell check_var: " + (NormalizeDouble(Bid, Digits)
                        + ", " + lowest_price + "," + -StepPip*Point));
               if (NormalizeDouble(Bid, Digits) -  lowest_price <= -StepPip*Point || lowest_price == -1) {
