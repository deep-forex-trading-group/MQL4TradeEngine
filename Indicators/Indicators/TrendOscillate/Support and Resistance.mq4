...
bool MoveObject(string name, int type, datetime Atime, double Aprice,
                datetime Atime2 = 0, double Aprice2 = 0,
                color Acolor = CLR_NONE, int Aweight = 0, int Astyle = 0) {
    if (ObjectFind(name) != -1) {
        ...
        return(ObjectCreate(name, type, 0, Atime, Aprice, Atime2, Aprice2, 0, 0)
               && ObjectSet(name, OBJPROP_COLOR, Acolor));
    }
}
...
int start() {
    ...
    for(shift=Bars-1;shift>=0 ;shift--){
        ...
        if( Close[shift+1]<sup ) {
            MoveObject("trade",OBJ_FIBO,Time[shift+1],sup,Time[shift+1],res,Green,1,STYLE_DASH);
        } else if( Close[shift+1]>res ) {
            MoveObject("trade",OBJ_FIBO,Time[shift+1],res,Time[shift+1],sup,Green,1,STYLE_DASH);
        } else if( Close[shift+1]<res && Close[shift+1]>sup ) {
            MoveObject("trade",OBJ_FIBO,Time[shift+1],res,Time[shift+1],sup,Green,1,STYLE_DASH);
        }
    }
