...
int OnInit(void) {
    ...
    if(InpFastEMA<=1 || InpSlowEMA<=1
       || InpSignalSMA<=1 || InpFastEMA>=InpSlowEMA) {
...
int OnCalculate (const int rates_total,
    ...
    for(i=0; i<limit; i++) {
        ima_fast = iMA(NULL,0,InpFastEMA,0,MODE_EMA,PRICE_CLOSE,i);
        ima_slow = iMA(NULL,0,InpSlowEMA,0,MODE_EMA,PRICE_CLOSE,i);
        ExtMacdBuffer[i] = ima_fast - ima_slow;
    }
